function [p,pgp,ASYMCORR] = sgd_pdf_metric_sobostat(S,H,H_interpolant,opts,data_folder)
...
% Promotion/Demotion stochasticity (NEW)
if isfield(opts,'swap_noise_amp'), 		 swap_noise_amp = opts.swap_noise_amp; else swap_noise_amp = 0.2 * S.rp; end   % << conservative default
if isfield(opts,'swap_noise_tau'),       swap_noise_tau = opts.swap_noise_tau; else swap_noise_tau = 10; end         % decorrelation time in steps
...
if S.pot_corr
	...
	% --- STOCHASTIC PROMOTION STATE (NEW) ---
	persistent swap_noise
	if isempty(swap_noise)
		swap_noise = zeros(S.N,1);
	end
...
while true
	...
	% --- UPDATE STOCHASTIC PROMOTION NOISE (NEW) ---
	alpha = exp(-1 / swap_noise_tau);
	swap_noise = alpha * swap_noise ...
           + sqrt(1 - alpha^2) * swap_noise_amp .* randn(S.N,1);
	% Noisy effective boundary (zero-mean)
	R_eff = S.br + swap_noise;
        ...
        if steps_in_batch >= sgd_batch_size
            ...
            % --- B. PLOT (Now that history exists) ---
             if graphing
                ...
                % Add a legend so you know what is what
                if numel(history.steps) == 1
                     legend('Metric', 'Target', 'Location', 'northeast', 'Color', 'k', 'TextColor', 'w');
                end
	...
	% --- PROMOTION AND DEMOTION (STOCHASTICIZED) ---------------------
	p2rho = vecnorm(p2,2,2); 
	pgp2rho = vecnorm(pgp2,2,2);

	

	% Swap decision (exclusive by construction)
	idx_swap = p2rho > R_eff;
