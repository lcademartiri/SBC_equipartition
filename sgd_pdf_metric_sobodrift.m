function [p,pgp,ASYMCORR] = sgd_pdf_metric_sobodrift(S,H,H_interpolant,opts,data_folder)
...
C.pos=[0;0;0]        % 3x1 correction center
C.D = S.esdiff/10;   % diffusion coefficient of center (slow!)
C.stdx=sqrt(2*C.D*relaxsteps*S.timestep);         % x diffusion step of center
C.Rmax=S.br-S.rc;       % max allowed |C|
% ------------------------------------------
...
while true
	...
	C.pos = C.pos + C.stdx * randn(3,1);
	if norm(C.pos) > C.Rmax
		C.pos = C.pos * (C.Rmax / norm(C.pos));
	end
	% --------------------------------------------------------------
	...
	% calculate real particle versors with respect to drifting center
	pvers_drift = (p - C.pos') ./ (vecnorm(p - C.pos',2,2)+eps);
	% add corrections to the real particle displacements
    total_disp = base_disp + (dr_corr_mag .* pvers_drift);
