...
while flag==1
    % --- DENSITY-AWARE LATTICE SPACING ---
    % If Phi is low, inflate the lattice so particles fill the sphere at t=0.
    % We use 0.50 as a reference "dense" packing fraction.
    lattice_scale = max(1, (0.50 / S.phi)^(1/3));
    
    basis=[0,0.7071,0.7071;0.7071,0,0.7071;0.7071,0.7071,0].*(2.01 * S.rp * lattice_scale);
    
    % Adjust search grid to account for the larger spacing
...
if graphing && S.pot_corr
    ...
    ax_rad_snr = subplot(3,2,6); % <--- NEW: 6th subplot for Radial SNR
    ...
    if thermflag == 1
        ...
        if steps_in_batch >= sgd_batch_size
            ...
            for ii = idxs'
                ...
                bin_mean(ii) = mu;
                snr(ii) = s;
                mu_sigma(ii) = se;   % <-- THIS IS THE KEY LINE
            end
            ...
            if batches_in_stage > current_grace_limit
                % v10 convergence: drift consistency
                max_snr = max(snr(has_data));
                
                if max_snr < snr_target
            ...
            % --- B. PLOT (Now that history exists) ---
             if graphing
                ...
                % --- NEW: 6. Radial SNR Profile (Bottom Right) ---
                subplot(ax_rad_snr);
                % Plot SNR vs Distance from Center
                plot(sgd_centers, snr, 'c.-', 'LineWidth', 1.5);
                
                % Add a reference line for your target SNR
                yline(snr_target, '--r', 'Target');
                
                title('Radial SNR Profile','Color','w');
                xlabel('Radius (r)','Color','w');
                xlim([min(sgd_centers), max(sgd_centers)]);
                set(gca,'Color','k','XColor','w','YColor','w','LineWidth',2);

                drawnow;
                
                msg = sprintf('Step %d | RMS: %.4f (Target: %.4f) | Pass: %d/%d | Gain: %.1e', ...
                    qs, dens_metric, rms_pass_counter, required_passes, sgd_gain);
            ...
            if transition_triggered && ~is_frozen_production
                ...
                if next_batch_size > max_batch_size_limit
                    ...
                    theoretical_sigma_next = sqrt(mean(mu_sigma.^2));
                    theoretical_sigma = max(theoretical_sigma, theoretical_sigma_next);
...
ASYMCORR.history = history;
ASYMCORR.sgd_edges = sgd_edges;
ASYMCORR.S=S;
ASYMCORR.opts=opts;
