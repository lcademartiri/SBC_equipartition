function [p,pgp,ASYMCORR] = sgd_pdf_metric_driftingcenter(S,PDF,H,H_interpolant,opts,data_folder)
...
C.D = 1e-4 * S.esdiff;   % diffusion coefficient of center (slow!)
C.stdx=sqrt(2*C.D*relaxsteps*S.timestep);         % x diffusion step of center
C.Rmax=2*S.rp;       % max allowed |C|
% ------------------------------------------
